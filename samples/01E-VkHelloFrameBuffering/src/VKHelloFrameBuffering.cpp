...
#include "VKHelloFrameBuffering.hpp"
#include "VKDebug.h"

VKHelloFrameBuffering::VKHelloFrameBuffering(uint32_t width, uint32_t height, std::string name) :
VKSample(width, height, name)
{
}

void VKHelloFrameBuffering::OnInit()
{
    InitVulkan();
    SetupPipeline();
}

void VKHelloFrameBuffering::InitVulkan()
{
    CreateInstance();
    CreateSurface();
...
void VKHelloFrameBuffering::SetupPipeline()
{
    CreateVertexBuffer();
...
void VKHelloFrameBuffering::OnUpdate()
{
...
void VKHelloFrameBuffering::OnDestroy()
{
    m_initialized = false;
...
void VKHelloFrameBuffering::CreateVertexBuffer()
{
...
void VKHelloFrameBuffering::CreateDescriptorSetLayout()
{
    //
...
void VKHelloFrameBuffering::CreatePipelineLayout()
{
...
void VKHelloFrameBuffering::CreatePipelineObjects()
{
    //
...
void VKHelloFrameBuffering::PopulateCommandBuffer(uint32_t currentImageIndex)
{
    ...
    clearValues[0].color = { { 0.5f, 0.2f, 1.0f, 1.0f } };
