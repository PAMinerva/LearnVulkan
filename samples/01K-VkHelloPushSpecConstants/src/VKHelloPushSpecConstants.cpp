...
#include "VKHelloPushSpecConstants.hpp"
#include "VKDebug.h"

#include "glm/gtc/matrix_transform.hpp"
#include "glm/ext/scalar_constants.hpp"

VKHelloPushSpecConstants::VKHelloPushSpecConstants(uint32_t width, uint32_t height, std::string name) :
...
m_dynamicUBOAlignment(0)
{
    ...
    m_dynUBufVS.meshInfo = nullptr;

    // Initialize the view matrix
    glm::vec3 c_pos = { 0.0f, -10.0f, 3.0f };
    glm::vec3 c_at =  { 0.0f, 0.0f, 1.0f };
    glm::vec3 c_down =  { 0.0f, 0.0f, -1.0f };
    m_uBufVS.viewMatrix = glm::lookAtLH(c_pos, c_at, c_down);

    // Initialize the projection matrix by setting the frustum information
    m_uBufVS.projectionMatrix = glm::perspectiveLH(glm::quarter_pi<float>(), (float)width/height, 0.01f, 100.0f);

    // Initialize the lighting parameters (directions and colors)
    m_pushConstants.lightDirs[0] = {-0.577f, -0.577f, 0.577f, 0.0f};
    m_pushConstants.lightDirs[1] = {0.0f, -1.0f, 0.0f, 0.0f};
    m_pushConstants.lightColors[0] = {0.9f, 0.9f, 0.9f, 1.0f};
    m_pushConstants.lightColors[1] = {0.8f, 0.0f, 0.0f, 1.0f};
}

VKHelloPushSpecConstants::~VKHelloPushSpecConstants()
{
    if (m_dynUBufVS.meshInfo)
        AlignedFree(m_dynUBufVS.meshInfo);
}

void VKHelloPushSpecConstants::OnInit()
{
    InitVulkan();
    SetupPipeline();
...
void VKHelloPushSpecConstants::InitVulkan()
{
    CreateInstance();
    CreateSurface();
...
void VKHelloPushSpecConstants::SetupPipeline()
{
    CreateVertexBuffer();
...
void VKHelloPushSpecConstants::OnUpdate()
{
    ...
    // Update buffer data (light direction, view and projection matrices)
    //UpdateHostVisibleBufferData();
}

// Render the scene.
void VKHelloPushSpecConstants::OnRender()
{
...
void VKHelloPushSpecConstants::OnDestroy()
{
    m_initialized = false;
...
void VKHelloPushSpecConstants::OnResize()
{
    // Recreate the projection matrix
    m_uBufVS.projectionMatrix = glm::perspectiveLH(glm::quarter_pi<float>(), (float)m_width/m_height, 0.01f, 100.0f);

    // Update uniform buffer data
    UpdateHostVisibleBufferData();
}

// Create vertex and index buffers describing a cube
void VKHelloPushSpecConstants::CreateVertexBuffer()
{
...
void VKHelloPushSpecConstants::CreateHostVisibleBuffers()
{
    //
    ...
    bufferInfo.size = sizeof(m_uBufVS);
    ...
    for (size_t i = 0; i < MAX_FRAME_LAG; i++)
    {
        ...
        m_sampleParams.FrameRes.HostVisibleBuffers[i].Descriptor.range = sizeof(m_uBufVS);
    }
}

void VKHelloPushSpecConstants::CreateHostVisibleDynamicBuffers()
{
    ...
    m_dynUBufVS.meshInfo = (MeshInfo*)AlignedAlloc(dynBufferSize, m_dynamicUBOAlignment);
	assert(m_dynUBufVS.meshInfo);
...
void VKHelloPushSpecConstants::UpdateHostVisibleBufferData()
{
    // Update uniform buffer data
    // Note: Since we requested a host coherent memory type for the uniform buffer, the write is instantly visible to the GPU
    for (size_t i = 0; i < MAX_FRAME_LAG; i++)
        memcpy(m_sampleParams.FrameRes.HostVisibleBuffers[i].MappedMemory, &m_uBufVS, sizeof(m_uBufVS));
}

void VKHelloPushSpecConstants::UpdateHostVisibleDynamicBufferData()
{
    ...
    {
        MeshInfo* mesh_info = (MeshInfo*)((uint64_t)m_dynUBufVS.meshInfo + (i * m_dynamicUBOAlignment));
        ...
        else
        {
            ...
            glm::mat4 Tran = glm::translate(glm::identity<glm::mat4>(), 5.0f * glm::vec3(m_pushConstants.lightDirs[i-1]));
            glm::mat4 Scale = glm::scale(glm::mat4(1.0f), glm::vec3(0.2f, 0.2f, 0.2f));
            mesh_info->worldMatrix = Tran * Scale;
            mesh_info->solidColor = m_pushConstants.lightColors[i-1];
        }
    }
    ...
    memcpy(m_sampleParams.FrameRes.HostVisibleDynamicBuffers[m_frameIndex].MappedMemory,
           m_dynUBufVS.meshInfo, 
           m_sampleParams.FrameRes.HostVisibleDynamicBuffers[m_frameIndex].Size);
}

void VKHelloPushSpecConstants::CreateDescriptorPool()
{
    //
...
void VKHelloPushSpecConstants::CreateDescriptorSetLayout()
{
    //
...
void VKHelloPushSpecConstants::AllocateDescriptorSets()
{
...
void VKHelloPushSpecConstants::SubmitCommandBuffer()
{
...
void VKHelloPushSpecConstants::PresentImage(uint32_t currentImageIndex)
{
